 ...
 */
const findLocalPackages = () =>
  spawn.sync("cross-env", [
...
Object.keys(pkgList)
  .filter(
    (pkg: any) => pkg.match("@hospitalrun") && !pkg.match("@hospitalrun/cli")
  )
  .forEach((pkg: string) => {
    try {
      let mod = require(pkg);
      ...
      if (typeof mod.commands !== "undefined") {
        Object.keys(mod.commands).forEach((cmd: any) => {
          program.command(cmd, mod.commands[cmd].description, {
